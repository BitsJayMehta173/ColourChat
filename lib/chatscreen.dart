...
class _ChatScreenState extends State<ChatScreen> {
  ...
  final DatabaseReference sendermessageRef = FirebaseDatabase.instance
      .ref()
      .child(
          '${globals.phonenumber}/friendlist/${globals.currfriend}/messages');
  final DatabaseReference receivermessageRef = FirebaseDatabase.instance
      .ref()
      .child(
          '${globals.currfriend}/friendlist/${globals.phonenumber}/messages');
  ...
  Future<void> checkFileExists() async {
    try {
      ...
      final path = directory.path;
      final file = File(
          '$path/$currfriend.txt'); // Replace 'your_filename.txt' with the actual file name
      ...
      data.forEach((key, value) {
        if (!vis.containsKey(value['timestamp'])) {
          ...
          if (value['phonenumber'] == globals.phonenumber) {
            String mes = "";
            for (int i = 0; i < value['text'].length; i += 4) {
              // Extract 4 characters from 'text' starting at index i
              String substring = value['text'].substring(i,
                  i + 4 <= value['text'].length ? i + 4 : value['text'].length);

              // Check if the substring exists in reversekeytable
              if (reversekeytablee.containsKey(substring)) {
                mes += reversekeytablee[substring] ??
                    ""; // Use an empty string if the value is null
              }
            }

            _messages.add(
                Message(text: mes, isSentByMe: value['isSentByMe']));
          } else {
            String mes = "";
            for (int i = 0; i < value['text'].length; i += 4) {
              // Extract 4 characters from 'text' starting at index i
            ...
            _messages.add(Message(text: mes, isSentByMe: false));
          }
        }
      });
      setState(() {
